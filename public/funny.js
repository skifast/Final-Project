(function() {
     ...
     * @returns 
     */
    async function fetchFunnyCostumes(){
        const url = BASE_URL + "costumes/Funny";
        try{
            let resp = await fetch(url);
            resp = await checkStatus(resp);
            const data = await resp.json();
            displayFunnyCostumes(data);
        } catch (err) {
            handleError(err);
            return;
        }
     ...
     * @param {Array} data 
     */
    function displayFunnyCostumes(data){
        ...
        data.forEach(costume => {
            ...
            // Add onclick event listener to each tile
